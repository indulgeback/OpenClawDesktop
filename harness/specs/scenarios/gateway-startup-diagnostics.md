...
Use this spec when OpenClawPro shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

OpenClawPro should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update OpenClawPro's main-process capability cache.
...
- **Handshake ready** only means OpenClawPro connected to the Gateway socket.
...
2. Read recent OpenClawPro logs:

```bash
tail -n 160 "$HOME/Library/Application Support/openclawpro/logs/openclawpro-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/openclawpro-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/openclawpro-health.json
pnpm exec openclaw gateway call status >/tmp/openclawpro-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/openclawpro-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/openclawpro-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/openclawpro-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/openclawpro-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/openclawpro-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the OpenClawPro-owned Gateway child.
...
- Whether the finding points to OpenClawPro-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Explain to users that restart cost is dominated by active Gateway work, not by OpenClawPro UI rendering.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries OpenClawPro can safely clean.
...
- Separate OpenClawPro-owned cleanup issues from OpenClaw runtime initialization cost.

...
async function launchOpenClawProElectron(
  homeDir: string,
  userDataDir: string,
  ...
  return await electron.launch({
    ...
    env: {
      ...
      OPENCLAWPRO_E2E: '1',
      OPENCLAWPRO_USER_DATA_DIR: userDataDir,
      ...(options.skipSetup ? { OPENCLAWPRO_E2E_SKIP_SETUP: '1' } : {}),
      OPENCLAWPRO_PORT_OPENCLAWPRO_HOST_API: String(hostApiPort),
    },
    timeout: 90_000,
  });
}
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'openclawpro-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'openclawpro-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchOpenClawProElectron(homeDir, userDataDir, options));
  },

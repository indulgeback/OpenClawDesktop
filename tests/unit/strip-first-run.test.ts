...
const { testHome } = vi.hoisted(() => ({
  testHome: `/tmp/openclawpro-openclaw-workspace-${Math.random().toString(36).slice(2)}`,
}));
...
describe('stripFirstRunSection', () => {
  ...
  it('still changes AGENTS content when only First Run is removed', () => {
    ...
    const merged = mergeOpenClawProSection(stripped, section);

    expect(merged).not.toBe(original);
    expect(merged).not.toContain('## First Run');
    expect(merged).toContain('## Session Startup');
    expect(merged).toContain('<!-- openclawpro:begin -->');
    expect(merged).toContain('<!-- openclawpro:end -->');
  });
});
...
describe('ensureOpenClawProContext', () => {
  it('does not wait for missing files in non-default agent workspaces', async () => {
    ...
    const result = await Promise.race([
      ensureOpenClawProContext().then(() => 'done'),
      new Promise((resolve) => setTimeout(() => resolve('timeout'), 200)),
    ]);

    expect(result).toBe('done');
    await expect(readFile(join(defaultWorkspace, 'AGENTS.md'), 'utf-8')).resolves.toContain('## OpenClawPro Environment');
    await expect(readFile(join(defaultWorkspace, 'TOOLS.md'), 'utf-8')).resolves.toContain('## OpenClawPro Tool Notes');

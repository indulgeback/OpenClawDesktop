...
const testOpenClawConfigDir = join(tmpdir(), 'openclawpro-tests', 'diagnostics-routes-openclaw');
...
describe('handleDiagnosticsRoutes', () => {
  beforeEach(() => {
    ...
    readLogFileMock.mockResolvedValue('openclawpro-log-tail');
  });
  ...
  it('returns diagnostics snapshot with channel view and tailed logs', async () => {
    ...
    expect(payload.openclawproLogTail).toBe('openclawpro-log-tail');

 ...
 * Used by OpenClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the OpenClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged OpenClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  OpenClaw sets
 ...
 * OpenClaw/OpenClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When OpenClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for OpenClaw to properly integrate with its updated tool system.
    ...
    // but OpenClaw historically stored them only under `channels.<type>.accounts.default`.

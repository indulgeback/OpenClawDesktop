 ...
 *
 * This intentionally reads auth-profiles.json rather than OpenClawPro's provider
 ...
 * Used by OpenClawPro to seed the provider store when it's empty but providers are
...
/**
 * Write the OpenClawPro gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged OpenClawPro loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  OpenClawPro sets
 ...
 * OpenClaw/OpenClawPro versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // for OpenClawPro to properly integrate with its updated tool system.
    ...
    // OpenClawPro is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for OpenClawPro desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // OpenClawPro-critical bundled plugins, active provider plugins, and explicitly

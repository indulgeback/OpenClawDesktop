 ...
 * the Electron IPC system to display UI in the OpenClaw frontend.
 */
...
class DeviceOAuthManager extends EventEmitter {
  ...
  private async runMiniMaxFlow(
    ...
    const token: MiniMaxOAuthToken = await this.runWithProxyAwareFetch(() =>
      loginMiniMaxPortalOAuth({
        region,
        openUrl: async (url) => {
  ...
  private async runQwenFlow(): Promise<void> {
    ...
    const token: QwenOAuthToken = await this.runWithProxyAwareFetch(() =>
      loginQwenPortalOAuth({
        openUrl: async (url) => {
  ...
  private async onSuccess(
    ...
    // 3. Save provider record in OpenClaw's own store so UI shows it as configured

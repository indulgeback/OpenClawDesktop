...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'openclawpro-extensions.json');
  }
  return join(app.getAppPath(), 'openclawpro-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No openclawpro-extensions.json found, loading all builtin extensions');
  }

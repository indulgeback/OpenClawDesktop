...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[openclawpro-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[openclawpro-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (shouldFallbackFromModelsProbe(modelsResult)) {
    console.log(
      `[openclawpro-validate] ${providerType} /models returned ${modelsResult.status}, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    console.log(
      `[openclawpro-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );

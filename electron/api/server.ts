...
export function startHostApiServer(ctx: HostApiContext, port = getPort('OPENCLAWPRO_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set OPENCLAWPRO_PORT_OPENCLAWPRO_HOST_API env var to override the default port.`,
      );
    } else {

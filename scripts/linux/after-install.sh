...
# Post-installation script for OpenClawPro on Linux
...
# Create symbolic link for OpenClawPro app binary
if [ -x /opt/OpenClawPro/openclawpro ]; then
    ln -sf /opt/OpenClawPro/openclawpro /usr/local/bin/openclawpro 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OpenClawPro/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OpenClawPro/chrome-sandbox' || true
else
    chmod 0755 '/opt/OpenClawPro/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OpenClawPro/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/openclawpro'
...
echo "OpenClawPro has been installed successfully."

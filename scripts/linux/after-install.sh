...
# Post-installation script for OpenClaw on Linux
...
# Create symbolic link for OpenClaw app binary
if [ -x /opt/OpenClaw/openclawpro ]; then
    ln -sf /opt/OpenClaw/openclawpro /usr/local/bin/openclawpro 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OpenClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OpenClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/OpenClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OpenClaw/resources/apparmor-profile'
...
echo "OpenClaw has been installed successfully."

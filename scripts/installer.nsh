; OpenClawPro Custom NSIS Installer/Uninstaller Script
;
...
!macro customUnInstall
  ...
  _cu_removeData:
    ...
    RMDir /r "$LOCALAPPDATA\openclawpro"
    RMDir /r "$APPDATA\openclawpro"
    ...
    ; Check AppData\Local\openclawpro
    IfFileExists "$LOCALAPPDATA\openclawpro\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\openclawpro"
      IfFileExists "$LOCALAPPDATA\openclawpro\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\openclawpro"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\openclawpro
    IfFileExists "$APPDATA\openclawpro\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\openclawpro"
      IfFileExists "$APPDATA\openclawpro\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\openclawpro"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    IfFileExists "$PROFILE\.openclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $PROFILE\.openclaw"
    IfFileExists "$LOCALAPPDATA\openclawpro\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\openclawpro"
    IfFileExists "$APPDATA\openclawpro\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\openclawpro"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R2\.openclaw"
    RMDir /r "$R2\AppData\Local\openclawpro"
    RMDir /r "$R2\AppData\Roaming\openclawpro"
